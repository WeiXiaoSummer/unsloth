...
def CohereAttention_fast_forward_inference(
    ...
    if do_prefill:
        ...
        if attention_size != self.hidden_size:
            self.temp_O = torch.empty((1, bsz, self.hidden_size), dtype = dtype, device = "cuda")
        else:
        ...
        if self.use_qk_norm:
            self.q_norm_out_weight = torch.empty(self.q_norm.weight.shape, dtype = torch.float32, device = "cuda")
            self.k_norm_out_weight = torch.empty(self.k_norm.weight.shape, dtype = torch.float32, device = "cuda")
        else:
